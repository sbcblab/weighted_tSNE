...
dataset_file   = "DATA/keras2points.csv" # path to data file (must be .csv, features as columns, first row and first column are labels, first column after labels should contain the classes or target values)
weights_file   = "DATA/forensics/Forensics/eye+skin-color/eyeohtree.csv"
...
class_label    = "algorithm"              # label of the column with the classes or target values
...
dot_size       = 80
class_colors   = ['CYAN', 'GREEN', 'YELLOW', 'BEIGE', 'DRKBRW', 'BLACK'] # list of colors to be assigned to the classes in the data, options as below:
